...
pub async fn do_handshake<S>(
    stream: &mut S,
    role: HandshakeRole) -> std::io::Result<(ReadKey, WriteKey)>
where
    S: AsyncRead + AsyncWrite + Unpin // This combination of traits describes a bidirectional async stream,
{
    ...
    let secret_auth = match role {
        HandshakeRole::Client { server_public_key } => { 
            my_secret.diffie_hellman(&server_public_key)
        },
        HandshakeRole::Server { my_static_secret } => {
            my_static_secret.diffie_hellman(&peer_public)
        }
    };
    ...
    hkdf.expand(&info, &mut okm).map_err(|_| std::io::Error::new(std::io::ErrorKind::Other, "hkdf errror"))?;
...
mod tests {
    ...
    async fn test_handshake_success() {
        const SERVER_PUBKEY: [u8; 32] = [
            65, 176, 115, 118, 36, 147, 145, 126, 126, 120, 230, 223, 55, 244, 230, 112, 179, 181, 93, 11, 193, 222, 118, 29, 222, 26, 110, 228, 193, 155, 107, 124
        ];
        ...
        tokio::spawn(async move {
            ...
            server_socket.read_exact(&mut client_key_buf).await.expect("server failed to read client key");

            server_socket.write_all(&SERVER_PUBKEY).await.expect("server failed to write key");
        });
